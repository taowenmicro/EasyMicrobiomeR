...
Tax4Fun工具目前被Tax4Fun2所替带，这里示例内容省略，推荐使用编号4的代码运行Tax4Fun2内容。
...
com.file="./data/otutab.txt"
tree.file="./data/otus.tree"
treat.file="./data/metadata.tsv"
env.file="./data/env.txt"
save.wd="./2.Microbial community analysis/iCAMP/"
if(!dir.exists(save.wd)){fs::dir_create(save.wd)}

# nworker为线程数，memory.G为运行内存，根据分析数据量、计算资源和希望的运行时间自行设置。
prefix="Test"
rand.time=10
nworker=4
memory.G=30
...
if(!file.exists("./2.Microbial community analysis/iCAMP/pd.desc")) {
...
tem = paste0("./2.Microbial community analysis/",tem0)
...
if(file.exists(paste0("./2.Microbial community analysis/iCAMP/",prefix,".PhyloSignalSummary.csv"))){
  appendy=TRUE;col.namesy=FALSE
}else{
    appendy=FALSE;col.namesy=TRUE}


write.table(data.frame(ds=ds,n.min=bin.size.limit,binps$Index),file = paste0("./2.Microbial community analysis/iCAMP/",prefix,".PhyloSignalSummary.csv"),
            append = appendy, quote=FALSE, sep=",", row.names = FALSE,col.names = col.namesy)

if(file.exists(paste0("./2.Microbial community analysis/iCAMP/",prefix,".PhyloSignalDetail.csv"))){appendy2=TRUE;col.namesy2=FALSE
            }else{
...
treat.use = data.frame( as.numeric(map$Group)) %>%as.matrix()
...
write.csv(pnstout$index.grp,file = paste0("./2.Microbial community analysis/iCAMP/",prefix,".pNST.summary.",colnames(treat)[i],".csv"))
write.csv(pnstout$index.pair.grp,file = paste0("./2.Microbial community analysis/iCAMP/",prefix,".pNST.pairwise.",colnames(treat)[i],".csv"))

pnst.bt=NST::nst.boot(nst.result=pnstout, group=treat.use,
                      rand=rand.time, nworker=nworker)
write.csv(pnst.bt$NST.summary,file = paste0("./2.Microbial community analysis/iCAMP/",prefix,".pNST.bootstr.",colnames(treat)[i],".csv"))
write.csv(pnst.bt$NST.compare,file = paste0("./2.Microbial community analysis/iCAMP/",prefix,".pNST.compare.",colnames(treat)[i],".csv"))
...
write.csv(icboot$summary,file = paste0("./2.Microbial community analysis/iCAMP/",prefix,".iCAMP.BootSummary.",colnames(treat)[i],".csv"),row.names = FALSE)
write.csv(icboot$compare,file = paste0("./2.Microbial community analysis/iCAMP/",prefix,".iCAMP.Compare.",colnames(treat)[i],".csv"),row.names = FALSE)
