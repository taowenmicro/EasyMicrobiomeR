...
path_to_reference_data = "D:/public/Tax4Fun2/Tax4Fun2_ReferenceData_v2"
otudir = funcpath
#加载
library(Tax4Fun2)
#物种注释
...
rdacca.hp.micro <- function(ps,
  ...
  filename = paste(hpPath,'Mciro.rda.hp.csv',sep = "")
  write.csv(dat,filename)
  filename = paste(hpPath,'Micro.rda.hp.pdf',sep = "")
  ggsave(filename,p)
  filename = paste(hpPath,'Micro.rda.hp.jpg',sep = "")
  ggsave(filename,p)
  ...
  if (cca == TRUE) {
    ...
    filename = paste(hpPath,'Mciro.cca.hp.csv',sep = "")
    write.csv(dat,filename)
    filename = paste(hpPath,'Micro.cca.hp.pdf',sep = "")
    ggsave(filename,p)
    filename = paste(hpPath,'Micro.cca.hp.jpg',sep = "")
    ggsave(filename,p)
  }
  ...
  #输出层次分割结果
  dat = mite.cap.hp$Hier.part %>% as.data.frame() %>% arrange(desc(Individual))
  dat$id = row.names(dat)
  dat$id = factor(dat$id,levels = dat$id)
  p = ggplot(dat) + geom_bar(aes(x = id,y = Individual ),stat = "identity",colour="black",fill="#9ACD32" ) + theme_classic()
  p
  filename = paste(hpPath,'Mciro.dbRDA.hp.csv',sep = "")
  write.csv(dat,filename)
  filename = paste(hpPath,'Micro.dbRDA.hp.pdf',sep = "")
  ggsave(filename,p)
  filename = paste(hpPath,'Micro.dbRDA.hp.jpg',sep = "")
  ggsave(filename,p)
}
...
random.cor = function(pssub = ps,seed = seed){
  ...
  frimp.scaled1 <- rfPermute::rp.importance(frichness.rfP, scale = TRUE)%>% round(3)
  frimp.scaled1 <- frimp.scaled1[,1:2]
  colnames(frimp.scaled1) = c("PC1","PC1.p")
  NMDS1<- cbind(env,MDS1 = index$PC2)  
  frichness.rfP <- rfPermute::rfPermute(NMDS1$MDS1 ~., data=NMDS1, ntree = 500, na.action = na.omit, nrep = 100, num.cores = 1)
  frimp.scaled2 <- rfPermute::rp.importance(frichness.rfP, scale = TRUE) %>% round(3)
